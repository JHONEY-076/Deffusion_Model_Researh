...
def save_tsne_figure(real_x: np.ndarray, generated_sets: dict[str, np.ndarray], figure_dir: Path) -> None:
    ...
    def split_real_points_by_overlap(gen_points: np.ndarray, threshold: float = 1.4) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i, (name, generated_sample) in enumerate(sampled_generated_sets.items()):
        ...
        real_overlap, real_non_overlap = split_real_points_by_overlap(gen_points)
